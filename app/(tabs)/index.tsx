...
function StatTile({
  ...
  return (
    <View style={[styles.statTile, { borderColor, backgroundColor }]}>
      ...
      <ThemedText type="subtitle" selectable style={[styles.statValue, { color: valueColor }]}>
        {value}
      </ThemedText>
...
export default function HomeScreen() {
  ...
  const memberSince = new Date(currentUser.createdAt).toLocaleDateString(locale, {
    month: "short",
    day: "numeric",
    year: "numeric",
  });
  ...
  if (currentUser.role === "instructor") {
    ...
    const now = Date.now();
    const acceptedApplications = myApplications.filter((row) => row.status === "accepted");
    const completedApplications = acceptedApplications.filter(
      (row) => row.jobStatus === "completed",
    );
    const pendingApplications = myApplications.filter((row) => row.status === "pending");
    const upcomingSessions = acceptedApplications
      .filter((row) => row.startTime > now)
      .sort((a, b) => a.startTime - b.startTime)
      .slice(0, 3);
    const openMatches = availableJobs.filter((row) => !row.applicationStatus).length;

    const jobsTaken = acceptedApplications.length;
    const earnedAmount = completedApplications.reduce((sum, row) => sum + row.pay, 0);
    const acceptanceRate =
      myApplications.length === 0 ? 0 : Math.round((jobsTaken / myApplications.length) * 100);
    ...
    return (
      ...
      >
        ...
        >
          <View style={[styles.heroBubbleLarge, { backgroundColor: palette.primary }]} />
          <View style={[styles.heroBubbleSmall, { backgroundColor: palette.focusRing }]} />
        ...
        <BrandSurface>
          <View style={styles.sectionHeader}>
            <ThemedText type="subtitle">{t("home.instructor.nextTitle")}</ThemedText>
            <ThemedText style={{ color: palette.textMuted }}>
          ...
          {upcomingSessions.length === 0 ? (
            ...
            upcomingSessions.map((session) => (
              ...
              >
                <View style={styles.sessionHeader}>
                  ...
                  >
                    <ThemedText
                      ...
                      style={[styles.statusPillText, { color: palette.primary }]}
                    >
                ...
                <View style={styles.sessionHeader}>
                  <ThemedText type="defaultSemiBold" style={{ color: palette.success }} selectable>
  ...
  return (
    ...
    >
      <View style={styles.headerBlock}>
        ...
        <ThemedText style={{ color: palette.textMuted }}>{t("home.studio.subtitle")}</ThemedText>
      </View>
      ...
      <BrandSurface>
        ...
        {myStudioJobs.length === 0 ? (
          <ThemedText style={{ color: palette.textMuted }}>{t("home.studio.noRecent")}</ThemedText>
        ) : (
          myStudioJobs.slice(0, 3).map((job) => (
            <View key={job.jobId} style={[styles.sessionRow, { borderColor: palette.border }]}>
              <View style={styles.sessionHeader}>
                      ...
                      borderColor:
                        job.status === "open"
                          ...
                          : job.status === "cancelled"
                            ? palette.danger
                          : job.status === "completed" || job.status === "filled"
                            ? palette.success
                            : palette.border,
                      backgroundColor:
                        job.status === "open"
