...
import { getInstructorComplianceDetailsRead, instructorComplianceDetailsValidator } from "../compliance/instructor";
import { getStudioComplianceDetailsRead, studioComplianceDetailsValidator } from "../compliance/studio";
...
export const getMyStudioAccessSnapshot = query({
  ...
  handler: async (ctx) => {
    ...
    const studioIdentityStatus =
      studio.diditVerificationStatus === "approved"
        ? "approved"
        : studio.diditVerificationStatus === "declined"
          ? "declined"
          : studio.diditVerificationStatus === "pending" ||
              studio.diditVerificationStatus === "in_progress" ||
              studio.diditVerificationStatus === "in_review" ||
              studio.diditVerificationStatus === "abandoned" ||
              studio.diditVerificationStatus === "expired"
            ? "pending"
            : null;
      ...
      verification: buildDiditVerificationPayload(
        {
          ...
          sessionId: studio.diditSessionId ?? stripeAccount?.providerAccountId,
          ...
          statusRaw: studio.diditStatusRaw ?? stripeAccount?.status,
          lastEventAt: studio.diditLastEventAt ?? stripeAccount?.updatedAt,
          verifiedAt: studio.diditVerifiedAt ?? stripeAccount?.activatedAt,
          decision: studio.diditDecision ?? stripeAccount?.metadata,
        },
        internalAccess.verificationBypass,
      ),
    };
