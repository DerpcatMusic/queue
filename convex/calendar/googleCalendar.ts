...
const googleCalendarNodeInternal = (internal as unknown as { calendar: { googleCalendarNode: Record<string, unknown> } })
  .calendar.googleCalendarNode as any;
...
export const connectGoogleCalendarWithCode = action({
  ...
  handler: async (ctx, args) => {
    return await ctx.runAction(googleCalendarNodeInternal.connectGoogleCalendarWithCodeInternal, args);
  },
});
...
export const disconnectGoogleIntegrationLocally = internalMutation({
  ...
  returns: v.object({ ok: v.boolean() }),
  handler: async (ctx, args) => {
    const profile = (await ctx.runQuery(internal.calendar.googleCalendar.getCalendarProfileForUser, {
      userId: args.userId,
