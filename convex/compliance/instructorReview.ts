...
export const reviewInstructorCertificate = internalAction({
  ...
  handler: async (ctx, args): Promise<{ ok: boolean; status: string }> => {
    ...
    try {
      ...
      await ctx.runMutation(internal.compliance.instructor.markInstructorCertificateReviewProgress, {
        certificateId: args.certificateId,
      ...
      await ctx.runMutation(internal.compliance.instructor.applyInstructorCertificateReviewDecision, {
        certificateId: args.certificateId,
