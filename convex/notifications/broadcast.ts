...
export const sendJobNotifications = internalAction({
  ...
  handler: async (ctx, args) => {
    const payload = await ctx.runQuery(internal.notifications.core.getJobAndEligibleInstructors, {
      jobId: args.jobId,
    });
    ...
    if (results.length > 0) {
      await ctx.runMutation(internal.notifications.core.logDeliveryBatch, {
