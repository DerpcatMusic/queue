...
import { computePricingV2 } from "./pricing";
...
const mapStripeIdentitySessionStatusToLegacyIdentityStatus = (
  status: string,
...
export const syncStudioDiditVerificationV2 = internalMutation({
  args: {
    userId: v.id("users"),
    sessionId: v.string(),
    statusRaw: v.string(),
...
export const applyDiditStudioWebhookV2 = internalMutation({
  args: {
  ...
  handler: async (ctx, args) => {
    ...
    if (needsProcessing) {
      ...
      const user =
        userId && userId.length > 0 ? await ctx.db.get("users", userId as Id<"users">) : null;
      const studioProfile =
        user?.role === "studio"
          ? await ctx.db
              .query("studioProfiles")
...
export const syncStripeDestinationChargeFundSplitV2 = internalMutation({
  ...
  handler: async (ctx, args): Promise<any> => {
    ...
    return await ctx.runMutation(internal.payments.core.upsertFundSplitFromProviderV2, {
