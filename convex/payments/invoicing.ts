...
export const issueInvoiceForPayment = internalAction({
  ...
  handler: async (ctx, { paymentId }): Promise<InvoiceIssueResult> => {
    const context = (await ctx.runQuery(internal.payments.invoicing.getPaymentForInvoicing, {
      paymentId,
    ...
    const invoice = (await ctx.runMutation(internal.payments.invoicing.createInvoiceRecord, {
      paymentId,
      provider,
    ...
    try {
      ...
      await ctx.runMutation(internal.payments.invoicing.markInvoiceIssued, {
      ...
      await ctx.runMutation(internal.payments.invoicing.markInvoiceFailed, {
        invoiceId: invoice._id,
