...
function normalizeBranchUpdateArgs(args: {
  ...
  if (args.autoExpireMinutesBefore !== undefined) {
    ...
    if (!Number.isFinite(val) || !Number.isInteger(val) || val < 5 || val > 120 || val % 5 !== 0) {
...
export const createStudioBranch = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const h3Hierarchy = safeH3Hierarchy(normalized.latitude, normalized.longitude);
    if (h3Hierarchy) {
...
export const setPrimaryStudioBranch = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const branchH3 = safeH3Hierarchy(updatedBranch.latitude, updatedBranch.longitude);
    if (branchH3) {
