...
6. If running sandbox with `ALLOW_SANDBOX_DESTINATION_SELF_VERIFY=1`, mark destination verified in-app.
7. Tap `Withdraw to bank`.
8. Confirm payout transitions: `queued` -> `processing` -> `pending_provider`/`paid`.
