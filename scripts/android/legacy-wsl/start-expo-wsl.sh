...
purge_non_device_emulators() {
  local serials
  serials="$(list_non_device_emulators || true)"
  if [[ -z "$serials" ]]; then
    return 0
  fi
...
launch_dev_client() {
  ...
  if [[ -z "$app_id" ]]; then
    echo "Warning: expo.android.package is missing in app.json; skipping app launch."
    return 0
  fi

  # Try to launch dev client if already installed. Do not fail startup if missing.
...
launch_dev_client "$SERIAL"
purge_non_device_emulators

echo "Launching Expo (dev client mode, manual device launch)..."
