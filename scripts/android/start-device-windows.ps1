...
function Ensure-Reverse {
  param([string]$AdbExe, [string]$TargetSerial)
  foreach ($port in @(8081, 19000, 19001)) {
    try {
      & $AdbExe -s $TargetSerial reverse ("tcp:{0}" -f $port) ("tcp:{0}" -f $port) | Out-Null
    } catch {
      Write-Warning "Failed to adb reverse tcp:$port on $TargetSerial"
    }
  }
}
...
function Start-ExpoDevServer {
  if (Get-Command bunx -ErrorAction SilentlyContinue) {
    & bunx expo start --host lan --clear
    return
  }
...
try {
  ...
  Ensure-Reverse -AdbExe $adbExe -TargetSerial $Serial
  ...
  Install-DebugApk -AdbExe $adbExe -TargetSerial $Serial -ApkPath $apkPath
  ...
  # Start Metro without Expo Android device probing.
  Start-ExpoDevServer
} finally {
  Pop-Location
}
