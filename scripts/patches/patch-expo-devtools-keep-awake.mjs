...
for (const targetFile of targets) {
  ...
  if (original.includes("return () => {}; // patched: disable keep-awake")) {
    console.log(
      `[postinstall] Expo keep-awake patch already applied: ${path.basename(targetFile)}`,
    );
    continue;
  }
  ...
  if (!pattern.test(original)) {
    console.warn(
      `[postinstall] Skipping expo keep-awake patch: expected pattern not found in ${path.basename(targetFile)}.`,
    );
    continue;
  }

  const patched = original.replace(pattern, "return () => {}; // patched: disable keep-awake");
