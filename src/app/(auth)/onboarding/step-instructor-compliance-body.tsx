...
function getDocumentValue(
  reviewStatus: OnboardingComplianceCertificateRow["reviewStatus"] | OnboardingComplianceInsuranceRow["reviewStatus"] | undefined,
  t: ReturnType<typeof useTranslation>["t"]
): string {
  if (reviewStatus === "approved") return t("profile.compliance.values.approved");
  if (reviewStatus === "uploaded" || reviewStatus === "ai_pending" || reviewStatus === "ai_reviewing") {
...
function getInsuranceSubtitle(
  ...
  switch (row.reviewStatus) {
    ...
    case "uploaded": case "ai_pending": case "ai_reviewing": return t("profile.compliance.insurance.pendingBody");
    case "rejected": case "needs_resubmission": return t("profile.compliance.insurance.reuploadBody");
    default: return t("profile.compliance.insurance.missingBody");
  }
}
...
function getCertificateSubtitle(
  ...
  switch (row.reviewStatus) {
    ...
    case "uploaded": case "ai_pending": case "ai_reviewing": return t("profile.compliance.certificate.pendingBody");
    case "rejected": case "needs_resubmission": return t("profile.compliance.certificate.reuploadBody");
    default: return t("profile.compliance.certificate.missingBody");
  }
}
...
export function StepInstructorComplianceBody({
  ...
  return (
    <View style={[styles.verifyStage, { backgroundColor: color.surface, borderColor: color.borderStrong }]}>
      {verificationFeedback ? (
        <NoticeBanner tone={verificationFeedback.tone} message={verificationFeedback.message} onDismiss={() => setVerificationFeedback(null)} />
      ) : null}

      <View style={styles.sectionBlock}>
        <ThemedText type="subtitle">
          {complianceDetails.summary.canApplyToJobs ? t("profile.compliance.hero.readyTitle") : t("onboarding.verification.subtitle")}
        </ThemedText>
        <ThemedText style={{ color: color.textMuted }}>
      ...
      {!diditState.isVerified ? (
        <ActionButton label={t("profile.compliance.actions.startIdentity")} fullWidth {...(diditButtonColors ? { colors: diditButtonColors } : {})} onPress={() => router.replace("/instructor/profile/compliance")} />
      ) : null}
      ...
      <View style={[styles.verificationCard, { backgroundColor: color.surfaceAlt, borderColor: color.borderStrong }]}>
        <View style={styles.verificationCardHeader}>
          <ThemedText type="defaultSemiBold">{t("profile.compliance.sections.identity")}</ThemedText>
          <IdentityStatusBadge status={diditState.status} />
        </View>
        <ThemedText style={{ color: color.textMuted }}>
          {diditState.isVerified ? t("profile.compliance.identity.approved") : t("profile.compliance.identity.required")}
        </ThemedText>
        <ActionButton
          label={diditState.isVerified ? t("profile.navigation.identityVerification") : t("profile.compliance.actions.startIdentity")}
          fullWidth
      ...
      <View style={[styles.verificationCard, { backgroundColor: color.surfaceAlt, borderColor: color.borderStrong }]}>
        <View style={styles.verificationCardHeader}>
          <ThemedText type="defaultSemiBold">{t("profile.compliance.sections.insurance")}</ThemedText>
          <ThemedText type="caption" style={{ color: color.textMuted }}>{getDocumentValue(preferredInsurance?.reviewStatus, t)}</ThemedText>
        </View>
        <ThemedText style={{ color: color.textMuted }}>
          {getInsuranceSubtitle(preferredInsurance ?? null, i18n.resolvedLanguage ?? "en", t)}
        </ThemedText>
        <ActionButton
          label={complianceDetails.summary.hasApprovedInsurance ? t("profile.compliance.actions.replaceInsurance") : t("profile.compliance.actions.uploadInsurance")}
          fullWidth
          loading={isUploading}
      ...
      <View style={styles.sectionBlock}>
        <ThemedText type="defaultSemiBold">{t("profile.compliance.sections.certificates")}</ThemedText>
      </View>

      <View style={styles.verificationCardList}>
        <View style={[styles.verificationCard, { backgroundColor: color.surfaceAlt, borderColor: color.borderStrong }]}>
          <View style={styles.verificationCardHeader}>
            <ThemedText type="defaultSemiBold">{t("profile.compliance.certificate.title")}</ThemedText>
            <ThemedText type="caption" style={{ color: color.textMuted }}>{getDocumentValue(latestCertificate?.reviewStatus, t)}</ThemedText>
          </View>
          <ThemedText style={{ color: color.textMuted }}>
          ...
          return (
            <View key={sport} style={[styles.verificationCard, { backgroundColor: color.surfaceAlt, borderColor: color.borderStrong }]}>
              <View style={styles.verificationCardHeader}>
                ...
                <ThemedText type="caption" style={{ color: color.textMuted }}>{getDocumentValue(certificateRow?.reviewStatus, t)}</ThemedText>
              </View>
              <ThemedText style={{ color: color.textMuted }}>
      ...
      <View style={[styles.verificationCard, { backgroundColor: color.surfaceAlt, borderColor: color.borderStrong }]}>
        ...
        <ThemedText style={{ color: color.textMuted }}>
          {pushToken ? t("onboarding.verification.reviewUpdatesEnabled") : t("onboarding.verification.reviewUpdatesDisabled")}
        </ThemedText>
        {!pushToken ? (
          <ActionButton
            disabled={isRequestingPush}
            loading={isRequestingPush}
            label={isRequestingPush ? t("onboarding.push.requesting") : t("onboarding.push.requestPermission")}
            tone="secondary"
            fullWidth
      ...
      <View style={styles.verifyActions}>
        <ActionButton
          label={complianceDetails.summary.canApplyToJobs ? t("onboarding.verification.openJobsReady") : t("onboarding.verification.openJobsWhileReviewing")}
          fullWidth
