...
// High-contrast, highly visible solid input
function SolidInput({ value, onChangeText, placeholder, multiline = false, autoCapitalize = "none" }: any) {
  const theme = useTheme();
  const [isFocused, setIsFocused] = useState(false);
  
  const focusVal = useSharedValue(0);
  useEffect(() => {
    focusVal.value = withSpring(isFocused ? 1 : 0, { damping: 20, stiffness: 200 });
  }, [isFocused]);
  ...
  return (
    <Animated.View style={[styles.inputContainer, animatedStyle, multiline && styles.inputContainerMultiline]}>
      <TextInput
        value={value}
        ...
        style={[styles.input, { color: theme.color.text }, multiline && styles.inputMultiline]}
      />
    </Animated.View>
  );
}
...
export default function InstructorProfileScreen() {
  ...
  // Persist on change
  useEffect(() => { save({ displayName }); }, [displayName]);
  useEffect(() => { save({ bio }); }, [bio]);

  const handleContinue = () => {
    router.push("/onboarding/instructor/sports");
  };
    ...
    >
      <View style={styles.header}>
        <Animated.Text entering={FadeInDown.delay(100)} style={[styles.title, { color: theme.color.text }]}>
          {t("onboarding.profile.title", { defaultValue: "Tell us about yourself" })}
        </Animated.Text>
      </View>

      <View style={styles.form}>
        <Animated.View entering={FadeInDown.delay(200)}>
          <Text style={[styles.label, { color: theme.color.text }]}>{t("onboarding.fullName", { defaultValue: "Full name" })}</Text>
          <SolidInput
            value={displayName}
            onChangeText={setDisplayName}
            autoCapitalize="words"
            placeholder={t("onboarding.placeholders.fullName", { defaultValue: "Jane Doe" })}
          />
        </Animated.View>

        <Animated.View entering={FadeInDown.delay(300)}>
          <Text style={[styles.label, { color: theme.color.text }]}>{t("onboarding.shortBio", { defaultValue: "Short bio (Optional)" })}</Text>
          <SolidInput
            value={bio}
            onChangeText={setBio}
            multiline
            placeholder={t("onboarding.placeholders.bio", { defaultValue: "I specialize in Vinyasa and strength training..." })}
          />
        </Animated.View>
      </View>
