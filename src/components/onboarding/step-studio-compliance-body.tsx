...
interface StepStudioComplianceBodyProps {
  currentUser: { role?: string; fullName?: string | null; email?: string | null } | null | undefined;
  ...
  setStudioVatReportingType: (type: "osek_patur" | "osek_murshe" | "company" | "other" | null) => void;
...
export function StepStudioComplianceBody({ currentUser, studioDiditVerification, onboardingStudioCompliance, verificationFeedback, setVerificationFeedback, isStartingStudioDidit, isSavingStudioBilling, studioLegalEntityType, setStudioLegalEntityType, studioVatReportingType, setStudioVatReportingType, studioLegalBusinessName, setStudioLegalBusinessName, studioTaxId, setStudioTaxId, studioBillingEmail, setStudioBillingEmail, studioBillingAddress, setStudioBillingAddress, router, buildRoleTabRoute, ROLE_TAB_ROUTE_NAMES, saveStudioBillingFromOnboarding, startStudioDiditFromOnboarding, refreshStudioDiditFromOnboarding, styles, }: StepStudioComplianceBodyProps) {
    ...
    return (
      <View style={styles.detailsLoadingStage}>
        <View style={styles.detailsLoadingHeader}>
          ...
          <ThemedText type="caption" style={{ color: color.textMuted }}>{t("onboarding.verification.studioBody")}</ThemedText>
        </View>
        <View style={styles.detailsLoadingRow}><ActivityIndicator color={color.primary} /><ThemedText style={{ color: color.textMuted }}>{t("profile.studioCompliance.loading")}</ThemedText></View>
      </View>
    );
  }
  ...
  const blockersSummary = getStudioBlockingSummary(studioComplianceDetails.summary.blockingReasons, t);
  ...
  return (
    <View style={[styles.verifyStage, { backgroundColor: color.surface, borderColor: color.borderStrong }]}>
      ...
      <View style={styles.sectionBlock}>
        <ThemedText type="subtitle">{studioComplianceDetails.summary.canPublishJobs ? t("profile.studioCompliance.hero.readyTitle") : t("profile.studioCompliance.hero.blockedTitle")}</ThemedText>
        <ThemedText style={{ color: color.textMuted }}>{studioComplianceDetails.summary.canPublishJobs ? t("profile.studioCompliance.hero.readyBody") : t("profile.studioCompliance.hero.blockedBody", { blockers: blockersSummary })}</ThemedText>
      </View>
      ...
      <View style={[styles.verificationCard, { backgroundColor: color.surfaceAlt, borderColor: color.borderStrong }]}>
        <View style={styles.sectionBlock}>
          <View style={styles.verificationCardHeader}><ThemedText type="defaultSemiBold">{t("profile.studioCompliance.sections.billing")}</ThemedText><ThemedText type="caption" style={{ color: color.textMuted }}>{studioComplianceDetails.summary.businessProfileStatus === "complete" ? t("profile.compliance.values.approved") : t("profile.compliance.values.actionRequired")}</ThemedText></View>
          <View style={styles.chipGrid}>
            <ChoicePill label={t("profile.studioCompliance.billing.entityIndividual")} selected={studioLegalEntityType === "individual"} onPress={() => setStudioLegalEntityType("individual")} />
            <ChoicePill label={t("profile.studioCompliance.billing.entityCompany")} selected={studioLegalEntityType === "company"} onPress={() => setStudioLegalEntityType("company")} />
          </View>
          <KitTextField label={t("profile.studioCompliance.billing.legalBusinessName")} value={studioLegalBusinessName} onChangeText={setStudioLegalBusinessName} />
          <KitTextField label={t("profile.studioCompliance.billing.taxId")} value={studioTaxId} onChangeText={setStudioTaxId} />
          <KitTextField label={t("profile.studioCompliance.billing.billingEmail")} value={studioBillingEmail} autoCapitalize="none" keyboardType="email-address" onChangeText={setStudioBillingEmail} />
          <KitTextField label={t("profile.studioCompliance.billing.billingAddress")} value={studioBillingAddress} onChangeText={setStudioBillingAddress} />
          <View style={styles.chipGrid}>{(["osek_patur", "osek_murshe", "company", "other"] as const).map((value) => <ChoicePill key={value} label={t(`profile.studioCompliance.billing.vatOptions.${value}` as const)} selected={studioVatReportingType === value} onPress={() => setStudioVatReportingType(value)} />)}</View>
          <ActionButton label={t("profile.studioCompliance.actions.saveBilling")} fullWidth loading={isSavingStudioBilling} disabled={isSavingStudioBilling} onPress={() => void saveStudioBillingFromOnboarding()} />
        </View>
      </View>
      <View style={[styles.verificationCard, { backgroundColor: color.surfaceAlt, borderColor: color.borderStrong }]}>
      ...
      <View style={styles.verifyActions}>
        <ActionButton label={studioComplianceDetails.summary.canPublishJobs ? t("onboarding.verification.openJobsReady") : t("onboarding.verification.openJobsWhileReviewing")} fullWidth onPress={() => router.replace(buildRoleTabRoute("studio", ROLE_TAB_ROUTE_NAMES.jobs))} />
        <ActionButton label={t("onboarding.verification.openCompliance")} tone="secondary" fullWidth onPress={() => router.replace("/studio/profile/compliance")} />
      </View>
    </View>
  );
